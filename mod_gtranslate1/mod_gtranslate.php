...
if($look == 'flags') {
        ...
        foreach($lang_array as $lang => $lang_name) {
                ...
                if($$show_this):
                        echo '<a href="javascript:doTranslate(\''.$language.'|'.$lang.'\')" title="'.$lang_name.'" style="font-size:'.$flag_size.'px;padding:1px 0;background:url(\'modules/flags/'.$flag_size.'.png\') no-repeat scroll -'.($i*100).'px -'.($j*100).'px;"><img src="modules/flags/blank.png" height="'.$flag_size.'" width="'.$flag_size.'" style="border:0;vertical-align:top;" alt="blank" /></a>';
                        if($orientation == 'v')
                                echo '<br />';
                        else
                                echo ' ';
